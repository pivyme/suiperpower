...
Do NOT implement any feature code. Do NOT modify `plans/`, CLAUDE.md, `bigdev/TODO.md`, or any source files. Do NOT mark TODO steps [x]. Do NOT commit. Output only a validation report.

## What counts as a BLOCKER (output `VALIDATION_BLOCKER`)

Only these:

1. **Baseline build is broken.** Run `pnpm install` if `node_modules` is missing, then `pnpm typecheck` (only if the script exists). If anything fails (compile errors, failing tests in code that already exists), that is a blocker. The loop's first action is a baseline check, it will spin trying to fix unrelated issues.

2. **A file referenced from CLAUDE.md does not exist.** Example: CLAUDE.md says "see `plans/05-SKILL-FORMAT.md`" but that file is missing.

3. **`bigdev/TODO.md` is empty or has no `[ ]` lines.** The loop will exit immediately with nothing to do.

4. **The build/test command(s) in CLAUDE.md cannot be executed at all** (e.g. `pnpm` is not installed and CLAUDE.md says use pnpm).
...
1. Read CLAUDE.md.
2. Read `bigdev/TODO.md`.
3. Run the build command(s) from CLAUDE.md exactly as written. If anything fails (and it's a real existing-code issue, not a missing-future-file issue), that's blocker 1.
4. Verify CLAUDE.md's plan-folder reference table points to files that exist (`ls plans/`).
...
- Do NOT commit.
- Do NOT modify `plans/`, CLAUDE.md, `bigdev/TODO.md`, or any source files.
