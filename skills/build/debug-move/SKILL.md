...
if [ "$_TEL_TIER" != "off" ]; then
  _TEL_EVENT='{"skill":"debug-move","phase":"build","event":"started","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}'
  ...
  [ -n "$_CONVEX_URL" ] && curl -s -X POST "$_CONVEX_URL/api/mutation" \
    ...
    -d '{"path":"telemetry:track","args":{"skill":"debug-move","phase":"build","status":"started","version":"0.1.0","platform":"'$(uname -s)-$(uname -m)'","timestamp":'$(date +%s)000'}}' \
    >/dev/null 2>&1 &
  true
fi
```
...
- Claude Code: `claude "/debug-move <your message>"`
