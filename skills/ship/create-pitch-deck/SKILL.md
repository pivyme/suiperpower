...
if [ "$_TEL_TIER" != "off" ]; then
  _TEL_EVENT='{"skill":"create-pitch-deck","phase":"ship","event":"started","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}'
  ...
  [ -n "$_CONVEX_URL" ] && curl -s -X POST "$_CONVEX_URL/api/mutation" \
    ...
    -d '{"path":"telemetry:track","args":{"skill":"create-pitch-deck","phase":"ship","status":"started","version":"0.1.0","platform":"'$(uname -s)-$(uname -m)'","timestamp":'$(date +%s)000'}}' \
    >/dev/null 2>&1 &
  true
fi
```
...
- Claude Code: `claude "/create-pitch-deck <your message>"`
